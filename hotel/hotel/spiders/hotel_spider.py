...
class hotelSpider(CrawlSpider):
    ...
    def parse(self,response):
        ...
        for u in urlList:
            ...
            s=r.decode('utf-8')
            try:
                j = json.loads(s.replace("\T","T"))
            except Exception as e:
                hotelfile = open("D:\\Users\\sangfor\\Desktop\\hotel.txt", 'a')
                print >> hotelfile, "error:%s"%(e)
                print >> hotelfile, "url=%s"%(url)
                hotelfile.close()
                continue
            total=j['hotelAmount']
            print ('****************************************************total:',total)
            if total%25==0:
                ...
                self.t=total/25+1
            while page<=self.t:
                # if cityId=='1'  and cityPY=='beijing' and (page == 8 or page == 43) :
                #     page +=1
                url="http://hotels.ctrip.com/Domestic/Tool/AjaxHotelList.aspx?cityId=%s&cityPY=%s&page=%d" %(cityId,cityPY,page)
                #url="http://hotels.ctrip.com/Domestic/Tool/AjaxHotelList.aspx?cityId=1&cityPY=beijing&page=9"
                r = requests.get(url=url).content.decode('utf-8')
                try:
                    j = json.loads(r.replace("\T","T"))
                except Exception as e:
                    f=open("D:\\Users\\sangfor\\Desktop\\hotel.txt",'a')
                    print >> f, "error:%s"%(e)
                    print >> f, "url=%s"%(url)
                    ...
                    for j in range(len(address_List)):
                        ...
                        print (
                        '***************************************************address_List[%d]' % (j), address_List[j])
