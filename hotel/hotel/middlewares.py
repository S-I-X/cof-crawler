...
def updateProxies():
    ...
    appKey = '133578743'
    secret = 'd5c02aef728b4ee9b9683d8fb75e4602'

    paramMap = {
        "app_key": appKey,
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")  # 如果你的程序在国外，请进行时区处理
    }
    print 'AppKey timestamp: ', paramMap['timestamp']
    # 排序
    keys = paramMap.keys()
    keys.sort()
...
class ProxyMiddleware(object):
    def process_request(self,request,spider):
        authHeader,proxies =updateProxies()
        request.meta['proxy'] ='http://s2.proxy.mayidaili.com:8123'
